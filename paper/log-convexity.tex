...
\title{Log-convexity of the exercise boundary of an American put option}
\author{Robert Martin\thanks{The proof was developed by GPT-6 Pro
  (Astra) and checked by Fable 5.1 (high). The result was formally verified in
  Lean, primarily by gpt-6-astra-high in Codex. The writeup was drafted
  with Codex and Claude Code (Fable 5.1). See \S\ref{sec:verification}
  and \ref{app:ai} for the verification scope and the use of AI.}\\
  Dianoa Labs}
...
\begin{abstract}
We prove that the logarithm of the exercise boundary of an American put
option is convex in time-to-expiry when the constant dividend yield is
nonnegative and does not exceed the positive risk-free interest rate. The proof
...
threshold varies with time-to-expiry. Although no general closed-form
...
Under the risk-neutral measure, let the stock-price process satisfy
\[
...
The bounds $(1-e^x)^+\le p(x,t)\le1$ and domination of the
European put value follow from discounted payoffs and the
admissibility of immediate exercise and exercise at expiry.
Increasing time-to-expiry enlarges the stopping-time class, so the
price is nondecreasing in $t$ and exercise sets shrink, making $b$
nonincreasing.
The formal development also derives these properties from its stopping
definitions; see \S\ref{sec:verification}.
Strict decrease is proved after convexity. No derivative of $b$ is
used until \S\ref{sec:classical}.
...
Fix $c>0$ and $d<0$, and let $\ell(t)=d-ct$. Define $f$ and $g$ by
...
\]
The comparison function need not be an option price. Its role is to
satisfy the pricing equation exactly while fitting a prescribed line.
...
We use a three-point maximum argument that preserves the interval
shape of every positive superlevel set. It requires only continuity
of the moving boundary and of the initial data.
...
forces the middle value to exceed $\varepsilon$ as well. This proves
the proposition, including exclusion of a zero between two positive
values. The argument uses no derivative of $b$ and no convergence
of spatial derivatives at expiry.
\end{proof}

The full zero-number theorem is unnecessary here. In particular,
Lou's moving-boundary theorem \cite[Theorem~1.2, condition (N)]{Lou2018}
allows continuous boundary curves when the solution is nonzero there;
unbounded boundary speed is not itself an obstruction in that case.
The direct proof above avoids both zero counting and its initialization
at expiry.
...
Lieberman \cite[Chapter~II]{Lieberman1996}. The boundary-point argument
needed here has the following direct proof.
...
obtain this positive point on the contact slice. The argument does
not assert that the boundary must remain below a line once it enters
that region.
...
The geometric theorem is separate from differentiability. The
positive-time regularity results of Chen--Chadam
\cite[Remark~2.2 and subsequent regularity analysis]{CC2007} for zero
dividends, and Chen--Cheng--Chadam \cite[Theorem~1 and \S3]{CCC2013}
for positive dividends, give $b\in C^2((0,\infty))$, in fact
...
The development builds on Mathlib \cite{Mathlib2020}, Degenne's BrownianMotion library,
...
\section{Use of AI}\label{app:ai}

The proof development, formalization, and manuscript preparation were
carried out largely by AI agents, with the author selecting the problem,
directing the investigation, and making editorial decisions. The
following is an account of that workflow, rather than an additional
argument for the theorem's correctness.

While researching American options, the author encountered the
unresolved dividend-paying boundary-convexity question and considered
it a promising subject for AI-assisted mathematical discovery.
The first task given to ChatGPT-6 Pro (astra) was to seek a
counterexample. After an extended search, Astra had not found one
and instead proposed structural reasons to expect convexity.
The author then asked it to prove the full conjecture in the
nonnegative-dividend regime $q\le r$.

After Astra produced a candidate proof, the author asked Anthropic's
Fable 5.1 to examine it for errors. That review reported no errors.
The project then moved to the Codex command-line environment for
formal verification in Lean, performed primarily by gpt-6-astra high,
with Fable 5.1 providing additional review during the development.
Repeated reviews by the same model are not independent checks.
The manuscript was drafted and revised with Codex and Claude Code
(Fable 5.1).

An unsuccessful counterexample search and agreement between models
do not establish a mathematical result. The evidence presented here
is the written argument and the kernel-checked formal development,
with the differences between them stated in
\S\ref{sec:verification}. The author is responsible for the claims
and presentation; the model reviews are not described as independent
human peer review.

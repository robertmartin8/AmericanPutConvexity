...
This is NOT the completed American-put curvature theorem. Three analytic
inputs remain explicit, unproved premises: negative boundary speed, the
near-expiry ratio limit, and the positive-interval invariant for decreasing
negative-intercept lines. Every remaining implication to curvature is proved
from the actual pricing-solution contract by the preceding modules.
-/
...
/-- The proposed log-curvature conclusion, conditional on the three remaining
...
/-- Zero dividends with the original CCJZ contract. The same three analytic
...
theorem zeroDividend_curvature_of_comparison_inputs {k : ℝ}
  ...
  dividend_curvature_of_comparison_inputs (dividendPutSolution_zero_iff.mpr hp) hspeed hexpiry hinterval
...
theorem stock_curvature_of_comparison_inputs {k h : ℝ}
  ...
  apply deriv2_stockBoundary_pos_of_nonneg hE hσ hT
    ...
    (dividend_curvature_of_comparison_inputs hp hspeed hexpiry hinterval _ ht)

import AmericanConvexity.Stopping.ActualDensityRightDerivative

/-! # The intrinsic actual heat flux has a right time derivative

The constructed density equals the actual one-sided spatial flux on the
neighborhood where the cutoff is one. Its proved right time derivative
therefore transfers to the intrinsic actual flux. No two-sided derivative
or continuity of the resulting time derivative is asserted here.
-/
...
theorem canonicalHeatThetaRightFlux_hasDerivWithinAt_right {k h t : ℝ}
    (hk : 0 < k) (hh : 0 ≤ h) (hhk : h ≤ k) (ht : 0 < t) :
    ∃ d : ℝ, HasDerivWithinAt (canonicalHeatThetaRightFlux k h) d (Ici (2*t)) (2*t) := by
  obtain ⟨D,hD,hDt,χ,hc,hcomp,hχ,hs,htrans,f,heq⟩ := exists_actualHeatSource_density hk hh hhk ht
  obtain ⟨d,hd⟩ := actualHeatSource_density_hasDerivWithinAt_right hk hh hhk hD hDt hc hcomp hχ hs htrans f heq
...
theorem zeroDividend_canonicalHeatThetaRightFlux_hasDerivWithinAt_right {k t : ℝ}
    (hk : 0 < k) (ht : 0 < t) :
    ∃ d : ℝ, HasDerivWithinAt (canonicalHeatThetaRightFlux k 0) d (Ici (2*t)) (2*t) :=
  canonicalHeatThetaRightFlux_hasDerivWithinAt_right hk le_rfl hk.le ht

theorem liuRange_canonicalHeatThetaRightFlux_hasDerivWithinAt_right {k h t : ℝ}
    (hh : 0 ≤ h) (hliu : h+1 ≤ k) (ht : 0 < t) :
    ∃ d : ℝ, HasDerivWithinAt (canonicalHeatThetaRightFlux k h) d (Ici (2*t)) (2*t) :=
  canonicalHeatThetaRightFlux_hasDerivWithinAt_right (by linarith) hh (by linarith) ht

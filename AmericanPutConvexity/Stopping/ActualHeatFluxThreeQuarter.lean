...
/-! # The intrinsic actual heat flux is locally three-quarter Holder

The improved density is identified with the actual one-sided spatial
derivative through the proved layer representation, on the neighborhood
where the cutoff is one. This transfers a modulus, not just continuity.
-/
...
theorem canonicalHeatThetaRightFlux_threeQuarter {k h t : ℝ}
    (hk : 0 < k) (hh : 0 ≤ h) (hhk : h ≤ k) (ht : 0 < t) :
    LocalThreeQuarterHolderAt (canonicalHeatThetaRightFlux k h) (2*t) := by
  obtain ⟨D,hD,hDt,χ,hc,hcomp,hχ,hs,htrans,f,heq⟩ := exists_actualHeatSource_density hk hh hhk ht
  have hholder := actualHeatSource_density_threeQuarter hk hh hhk hD hDt hc hcomp hχ hs htrans f heq
...
theorem zeroDividend_canonicalHeatThetaRightFlux_threeQuarter {k t : ℝ}
    (hk : 0 < k) (ht : 0 < t) :
    LocalThreeQuarterHolderAt (canonicalHeatThetaRightFlux k 0) (2*t) :=
  canonicalHeatThetaRightFlux_threeQuarter hk le_rfl hk.le ht

theorem liuRange_canonicalHeatThetaRightFlux_threeQuarter {k h t : ℝ}
    (hh : 0 ≤ h) (hliu : h+1 ≤ k) (ht : 0 < t) :
    LocalThreeQuarterHolderAt (canonicalHeatThetaRightFlux k h) (2*t) :=
  canonicalHeatThetaRightFlux_threeQuarter (by linarith) hh (by linarith) ht
